...
glColor4f(1,1,1,1);
		...
		glTexCoord2f(0.0f, 1.0f); glVertex3f( 1.0f, Pbaixo,  1.0f);
		glTexCoord2f(1.0f, 1.0f); glVertex3f(-1.0f, Pbaixo,  1.0f);
	    glEnd();
...
void DesenhaVoando(){
   ...
   /*
    Coord v[4];
    relevo.PegaQuadro(v,AviaoCorrente->posicao);
    GLfloat alt = relevo.PegaAltura(AviaoCorrente->posicao.x,AviaoCorrente->posicao.z ) ;   
    glPushMatrix();
    glTranslatef(AviaoCorrente->posicao.x,alt ,AviaoCorrente->posicao.z);
    ...
    glDisable(GL_LIGHTING); 
     ...
     glBegin(GL_LINES);
      glVertex3f(AviaoCorrente->posicao.x,alt ,AviaoCorrente->posicao.z);
      glVertex3f(AviaoCorrente->posicao.x,AviaoCorrente->posicao.y,AviaoCorrente->posicao.z);   
      glVertex3f(AviaoCorrente->posicao.x,alt ,AviaoCorrente->posicao.z);
      glVertex3f(v[0].x,v[0].y,v[0].z);    
      glVertex3f(AviaoCorrente->posicao.x,alt ,AviaoCorrente->posicao.z);
      glVertex3f(v[1].x,v[1].y,v[1].z);   
...
void idle ( void )
{
  ...
  if (DeltaTime>0.9)DeltaTime = 0.9;
  ...
  if (teclas['a'])  AviaoCorrente->setAilerons(90);
  else 
  if (teclas['d']) AviaoCorrente->setAilerons(-90); 
  else
  AviaoCorrente->setAilerons(0); 
  
  if (teclas['q']) AviaoCorrente->setLeme(-90);
  else 
  if (teclas['e']) AviaoCorrente->setLeme(90); 
  else
  AviaoCorrente->setLeme(0); 
  
  if (teclas['w']) AviaoCorrente->setProfundor(-90);
  else 
  if (teclas['s']) AviaoCorrente->setProfundor(90); 
  else
